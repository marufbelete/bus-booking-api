...
//birr
getGroupMonthAgentTicketInbr:async(parent,args,context)=>{
  try{
  const orgcode =context.organization_code;
  const allSchedule= await Schedule.aggregate( [
{
...
{
  $project:{"_id":0,"isMobileUser":{$arrayElemAt:["$user.isMobileUser",0]},"year":{$year:"$passangerInfo.bookedAt"},"month":{$month:"$passangerInfo.bookedAt"},"userRole":{$arrayElemAt:["$user.userRole",0]},"totalTicket":{$size:"$passangerInfo.passangerOccupiedSitNo"},"price":"$tarif"}
},
{
  $match:{"userRole":process.env.AGENT,"isMobileUser":false}
},
{
  $group:{_id:{"year":"$year","month":"$month"},"totalPrice":{$sum:{$multiple:["$totalTicket","$price"]}}}
},
{
  $project:{
    "_id":0,"year":"$_id.year","month":"$_id.month","totalPrice":1
  }
}
...
//birr
getGroupMonthLocalTicketInbr:async(parent,args,context)=>{
  try{
...
{
  $group:{_id:{"year":"$year","month":"$month"},"totalPrice":{$sum:{$multiply:["$totalTicket","$price"]}}}
},
{
  $project:{
    "_id":0,"year":"$_id.year","month":"$_id.month","totalPrice":1
...
//birr
getGroupMonthMobileTicketInbr:async(parent,args,context)=>{
  try{
  const orgcode =context.organization_code;
  const allSchedule= await Schedule.aggregate( [
{
